...
{:then data}
	{#if $user}
				...
				<Leaflet view={initialView} zoom={14}>
					{#each data as dataEntry}
						...
						<div class="flex flex-row w-64 gap-1 ml-2 transition" id="map-marker">
							...
							width={25}
							/>
							<p class="shadow-lg w-32 transition font-extrabold bg-slate-900 bg-opacity-70 p-1 mb-1 rounded-lg">{truncateString(dataEntry.name)}</p>
						</div>
		...
		<Icon class="text-6xl" icon="material-symbols-light:error" />
			...
			<a href="https://discord.gg/xjJCdtcFsQ" target="_blank" class="flex flex-row mt-1 items-center gap-1 border-2 p-1.5 rounded-lg border-slate-950 dark:border-slate-400 hover:border-blue-600 hover:bg-blue-600 transition">
				<Icon icon="ic:baseline-discord" class="text-xl"/>
				Meghívó
			</a>
			{:else}
			<h1 class="text-lg">A térkép elérése NotMe Discord szerverén legalább <b>Irógép</b> ranghoz kötött!</h1>
			<h2>Ha már megvan a rang, de a térképet még mindig nem tudod elérni, kérlek lépj be újra!</h2>
			{/if}
		</div>
		{/if}
	{:else}
